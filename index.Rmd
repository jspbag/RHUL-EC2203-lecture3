...
- **Explained variance**: variation in $\hat{Score}_i$ 
  ...
  Note: variation in $\hat{Score}_i = \hat{\beta}_0 + \hat{\beta}_1 STR_i$ is driven by variation in the regressor $STR_i$.
  
- **Residual variance**: variation in $\hat{u}_i$

  Note: $\hat{Score}_i$ and $\hat{u}_i$ are uncorrelated by virtue of OLS.
...
$$RSS = \sum_{i=1}^n \left(Y_i - \hat{Y}_i\right)^2 = \sum_{i=1}^n \hat{u}^2_i$$
...
- The $SER$, often denoted $s_{\hat{u}}$, is constructed from the regression residuals $(\hat{u}_i;i=1,2,...,n)$:
...
- $R^2 \in [0,1]$ with higher values indicating a better (in-sample) **fit** 

- The $R^2$ is the **share** of variation in $Score_i$ that is "explained" by $STR_i$.
...
- The $SER$ and the $R^2$ measures how close the dependent variable in the sample are to the in-sample prediction from the estimated model
